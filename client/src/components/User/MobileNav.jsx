...
const MobileNav = () => {
  ...
  return (
    <div className='absolute bottom-0 flex w-screen justify-evenly bg-neutral-800'>
        {nav.map((item) => <NavLink to={item.path} className={({isActive}) => isActive ? isActiveMobileNav : isNotActiveMobileNav}><i className='text-2xl'>{item.icon}</i><span className='text-sm'>{item.text}</span></NavLink>)}
